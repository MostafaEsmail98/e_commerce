...
import 'package:e_commrece/features/auth/data/dataSoures/remoteData/remote_sign_in_impl.dart';
...
class SignInCubit extends Cubit<SignInState> {
  ...
  var response = await  SignInUsesCase(
            signInRepo: SignInRepoImpl(
                remoteSignIn: RemoteSignInImpl(api: DioConsumer(dio: Dio()))))
        .call(params:SignInParams(email: emailController.text, password: passwordController.text));
  response.fold((failure)=>emit(SignInFailure(failure)), (user)=>emit(SignInSuccessful(user)));
  }
}
